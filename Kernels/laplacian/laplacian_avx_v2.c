...
int main(int argc, char** argv) 
{
	...
	do 
	{
		...
		for (j = 1; j < dim_m - 1; ++j)
		{
			...
			//print(st_read, dim_n, dim_m);
			//WEST  = _mm256_loadu_pd (vl - 1); 
			#pragma ivdep
			#pragma vector aligned
			for (; i + 8 < dim_n - 1; i = i + 8, count = count + 8)
			{
				//WEST  = _mm256_loadu_pd (vl - 1); 
				SOUTH1 = _mm256_loadu_pd (vl - dim_n); 
				NORTH1 = _mm256_loadu_pd (vl + dim_n); 
				EAST1  = _mm256_loadu_pd (vl + 1); 
				WEST1  = _mm256_loadu_pd (vl - 1); 
				//
				SOUTH2 = _mm256_loadu_pd (vl - dim_n + 4);
                                NORTH2 = _mm256_loadu_pd (vl + dim_n + 4);
                                EAST2  = _mm256_loadu_pd (vl + 1 + 4);
                                WEST2  = _mm256_loadu_pd (vl - 1 + 4);
				//
				AVX_STORE(vs + 0, alpha*(EAST1 + WEST1 + SOUTH1 + NORTH1)); 
                                AVX_STORE(vs + 4, alpha*(EAST2 + WEST2 + SOUTH2 + NORTH2));
