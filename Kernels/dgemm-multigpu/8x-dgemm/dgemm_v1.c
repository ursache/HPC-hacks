...
*/
	...
	for (ii = 0; ii < nGpuStreams; ii++)
	{
		...
		//cublasDgemm(handles[ii], 
		cublasDgemm( 
				//convertToOp(transa), convertToOp(transb),
				transa, transb,
				M_n,
				N_n, 
				K_n,
	...
	for (ii = 0; ii < nGpuStreams; ii++)
		...
		printf("GPU %d: C Data transfer rate = %f (%f)\n", jj, (M_n*N_n)*8*1e-9/datatime, datatime);
	}
	d2htime += mysecond();
	printf("D2H xfer time\t\t%f s.\n", d2htime);
	//
	 double updatetime = -mysecond();
	for (ii = 0; ii < nGpuStreams; ii++)
        {
		//@printf("Copying... %d %d %d, %f to %f\n ", mGPU[i - 1], nGPU[j - 1], kGPU[k - 1], *(C_h + 0), *(C + nGPU[j - 1]*ldc + mGPU[i - 1] + 0));
		//@printf("Copying... %d %d %d, %f to %f\n ", mGPU[i - 1], nGPU[j - 1], kGPU[k - 1], *(C_h + 1), *(C + nGPU[j - 1]*ldc + mGPU[i - 1] + 1));
		int jj = ii;
                //cublasSetStream(handles[jj], streams[jj]);
                //cudaSetDevice(jj);
                mapping(jj, &i, &j, &k);
		...
		int M_n      = mGPU[i] - mGPU[i - 1];
                ...
                int K_n      = kGPU[k] - kGPU[k - 1];
		//
		block_copy_add(C + nGPU[j - 1]*ldc + mGPU[i - 1], C_h[jj], beta, M_n, N_n, ldc, M_n);
		//
		cudaFree(C_h[jj]);
		cudaFree(d_A[jj]);
		cudaFree(d_B[jj]);
